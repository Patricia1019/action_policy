...
class BridgeDatasetTorch(IterableDataset):
    ...
    def __iter__(self):
        ...
        return sample_iterator # split to num workers
    ...
    def _sample_generator(self, start, end):
        ...
        for i, data_filename in enumerate(data_filenames):
            ...
            obs_imgs, obs_pros, next_obs_imgs, next_obs_pros, actions, terminals, truncates, goal_imgs, goal_pros, rewards, masks = self.convert_traj_to_samples(traj)
            # obs_imgs, next_obs_imgs, goal_imgs, actions = self.convert_traj_to_samples(traj)


            # convert to channel first and do augmentation

            # for obs_img, next_obs_img, goal_img, action in zip(
            #     obs_imgs, next_obs_imgs, goal_imgs, actions
            # ):
            #     obs_img = torch.tensor(obs_img).permute((2, 0, 1)) # convert to channel first
        ...
        yield obs_imgs, obs_pros, next_obs_imgs, next_obs_pros, actions, terminals, truncates, goal_imgs, goal_pros, rewards, masks
            # yield obs_imgs, next_obs_imgs, goal_imgs, actions
